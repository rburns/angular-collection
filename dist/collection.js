'use strict';
angular.module('collection', []).factory('collection', [
  'indexOfObj',
  'clone',
  '$parse',
  function (indexOfObj, _clone, $parse) {
    function clone() {
    ...
    function exc(prop, value, context) {
      ...
      if (context) {
        return this.not(context.clone().inc(prop, value));
      }
      if (angular.isArray(prop)) {
        return arrayExc(this, prop, context);
      }
      idx = this.length;
      while (idx--) {
        if (this[idx][prop] === value) {
          this.splice(idx, 1);
        }
      }
      return this;
    ...
    function inc(filter, value, context) {
      ...
      if (context) {
        return this.int(context.clone().inc(filter));
      }
      var idx = this.length;
      while (idx--) {
        if (and(this[idx], filter)) {
          this.splice(idx, 1);
        }
      }
      return this;
    ...
    function put(data) {
      if (angular.isArray(data)) {
        return arrayPut(this, data);
      }
      var idx = indexOfObj(this, 'key', data.key);
      if (idx === -1) {
        this.push(data);
      } else {
        this.splice(idx, 1, data);
      }
      return this;
    }
    ...
    function int(coll) {
      return this.inc(coll.map(function (obj) {
        return [
          'key',
          obj.key
        ];
    ...
    function not(coll) {
      return this.exc(coll.map(function (obj) {
        return [
          'key',
          obj.key
        ];
